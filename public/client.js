...
ws.onmessage = message => {
    ...
    for (const device in md.devices) {
        ...
        if (md.devices[device].commands) {
			md.devices[device].commands.forEach((command) => {
				if (!document.querySelector('#' + device + '-' + command.id)) {
					document.querySelector('#wrap-' + device + '-commands')
						.appendChild(createElement('div', {
					...
					document.querySelector('#' + device + '-' + command.id).addEventListener('mousedown', function(e) {
						...
						document.querySelector('#' + device + '-' + command.id).addEventListener('mouseup', function(e) {
